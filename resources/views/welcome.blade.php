...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="flex p-6 lg:p-8 items-center lg:justify-center min-h-screen flex-col bg-black text-white">
    
@include('components.my-nav')

<div class="items-center justify-center w-full transition-opacity opacity-100 duration-1500 lg:grow starting:opacity-0">

<section class="w-full bg-center bg-no-repeat bg-[url('/public/hero.png')] bg-cover bg-blend-multiply">
    <div class="px-4 mx-auto max-w-screen-xl text-center py-12 lg:py-36">
        <h1 class="mb-4 text-4xl font-extrabold tracking-tight leading-none text-black md:text-5xl lg:text-6xl bg-gray-100 p-2 rounded">Bilinçli Tüketim İçin Harekete Geç ✋</h1>
        <p class="mb-8 text-lg font-normal lg:text-xl sm:px-16 lg:px-48 bg-gray-100 p-2 rounded text-black">Güncel boykotları keşfedin, nedenlerini öğrenin ve alternatif markaları bulun. Değişime siz de katılın!</p>
        <div class="flex flex-col space-y-4 sm:flex-row sm:justify-center sm:space-y-0">
            <a href="{{route('boycotts.index')}}" class="inline-flex justify-center items-center py-3 px-5 text-base font-medium text-center text-white rounded-lg bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300">
                Tümünü Gör
...
{{-- Latest Boycotts Section --}}
<section class="py-12 md:py-20 w-full">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        <h2 class="text-3xl font-bold text-center mb-10 md:mb-12">
            Son Eklenen Boykotlar
        </h2>
        ...
        <div class="overflow-x-auto shadow-lg rounded-lg border border-gray-200">
            <table class="w-full table-auto text-left text-sm ">
                <thead class="text-xs uppercase bg-red-700 text-center">
                    <tr>
                        <th scope="col" class="px-6 py-4">Marka</th>
                        <th scope="col" class="px-6 py-4 hidden sm:table-cell">Sebep (Kısa)</th>
                        <th scope="col" class="px-6 py-4">Başlangıç / Bitiş</th>
                        <th scope="col" class="px-6 py-4 text-center">Destekçi</th>
                        <th scope="col" class="px-6 py-4 text-center">Detaylar</th>
                    </tr>
                </thead>
                <tbody>
                    @forelse ($latestBoycotts as $boycott)
                        <tr class="border-b hover:bg-gray-900 transition duration-150 ease-in-out text-center">
                            <td class="px-6 py-4 font-medium whitespace-nowrap">
                                {{ $boycott->brand->name }}
                            </td>
                            <td class="px-6 py-4 hidden sm:table-cell">
                                {{ Str::limit($boycott->reason, 80) }} {{-- Sebebi kısalt --}}
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                {{ $boycott->start_date->locale('tr')->translatedFormat('j F Y') }} {{-- Tarihi formatla (Carbon) --}}
                                <br>
                                {{ $boycott->end_date->locale('tr')->translatedFormat('j F Y') }} {{-- Tarihi formatla (Carbon) --}}
                            </td>
                            <td class="px-6 py-4 text-center font-semibold">
                                {{ $boycott->users_count }}
                            </td>
                            <td class="px-6 py-4 text-center">
                                <a href="{{ route('boycott.show', $boycott) }}"
                                   class="inline-flex justify-center items-center py-2 px-3 text-base font-medium text-center text-white rounded-lg bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300">
                                    İncele
                                </a>
                            </td>
                        </tr>
                    @empty
                        <tr class="bg-white dark:bg-gray-800 border-b dark:border-gray-700">
                            <td colspan="5" class="text-center px-6 py-10 text-gray-500 dark:text-gray-400">
                                Henüz gösterilecek boykot bulunmuyor.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        {{-- Tüm boykotları görme butonu (isteğe bağlı) --}}
        <div class="text-center mt-10">
             <a href="{{ route('boycotts.index') }}"
                class="inline-flex justify-center items-center py-3 px-5 text-base font-medium text-center text-white rounded-lg bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300">
                 Tüm Boykotları Gör
...
{{-- Bilinçli Tüketim İçin Harekete Geç
Güncel boykotları keşfedin, nedenlerini öğrenin ve alternatif markaları bulun. Değişime siz de katılın! --}}